...
contract XDToken is Ownable, ERC20("XD Token", "XD"), IXDToken {
  ...
  event UpdateAllocations(uint256 masterAllocation, uint256 treasuryAllocation);
   ...
   */
  modifier onlyMaster() {
    require(msg.sender == masterAddress, "XDToken: caller is not the master");
    _;
  }
  ...
  function updateAllocations(uint256 masterAllocation_) external onlyOwner {
    ...
    require(masterAllocation_ <= 100, "updateAllocations: total allocation is too high");
